...
b2*.py = English+ B2+/C1, fw1.py / fw2.py = English+ Function Words I / II —
those two are generated by export_quizzes.py in each function-words repo,
so edit their questions there and re-export). Edit a question, run `python3 build.py`, then
...
for m in ["site1", "site2", "site3", "b1a", "b1b", "b1c", "b2a", "b2b", "b2c", "fw1", "fw2"]:
    __import__(m)
Q = common.QUIZZES
...
groups = [
    ...
    ("english-plus-function-words-1", "seed-lesson-quizzes-english-plus-function-words-1.sql", "Lesson quizzes — English+ Function Words I, B1+/B2 (11 lessons)"),
    ("english-plus-function-words-2", "seed-lesson-quizzes-english-plus-function-words-2.sql", "Lesson quizzes — English+ Function Words II, B2+/C1 (11 lessons)"),
]
...
# Quizzes whose artwork hasn't been designed yet show the category
# placeholder on practice.html (see js/quiz-browse.js), so a missing file
# is reported, not fatal — but only for courses added after the artwork
# handoff (the function-words courses). Everything else must have its art.
NO_ART_YET = {"english-plus-function-words-1", "english-plus-function-words-2"}
missing_art = []
for q in Q:
    if not os.path.exists(os.path.join(here, "..", "..", "assets", "art", ART_SLUGS[q["key"]] + ".html")):
        assert q["course"] in NO_ART_YET, q["key"]
        missing_art.append(ART_SLUGS[q["key"]])
if missing_art:
    print(f"{len(missing_art)} quizzes use the placeholder artwork until assets/art/<slug>.html exists: {', '.join(missing_art)}")
...
art_sql = ["-- Migration 013: artwork for the 57 lesson quizzes",
           ...
           "", "begin;"]
for q in Q:
    if q["course"] in NO_ART_YET:
        continue  # migration 013 is the original 57; newer quizzes get their slug from their own seed file
